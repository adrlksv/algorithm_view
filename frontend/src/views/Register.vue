<template>
  <div class="flex flex-col items-center justify-center min-h-screen bg-black text-white p-6">
    <h1 class="text-4xl mb-6 neon-text">Регистрация</h1>
    <form @submit.prevent="handleRegister" class="w-full max-w-sm">
      <input v-model="username" type="text" placeholder="Имя пользователя" class="neon-input mb-4" />
      ...
      <button type="submit" class="neon-button w-full">Зарегистрироваться</button>
    </form>
    <p class="mt-4">
      Уже есть аккаунт?
      <router-link to="/login" class="text-blue-400 hover:underline">Войти</router-link>
    </p>
  </div>
</template>
