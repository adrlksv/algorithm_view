<template>
  <div class="min-h-screen bg-gray-900 text-gray-100 p-6 flex flex-col items-center justify-center">
    <div class="w-full max-w-md">
      <h1 class="text-4xl md:text-5xl font-bold mb-8 text-center text-green-400">Регистрация</h1>
      
      <form @submit.prevent="handleRegister" class="mb-6 p-6 rounded-xl bg-gray-800 border border-gray-700 shadow-md">
        <div class="mb-4">
          <label for="username" class="block mb-2 font-medium text-gray-300">Имя пользователя:</label>
          <input
            id="username"
            v-model="username"
            type="text"
      ...
      <p class="text-center text-gray-400">
        Уже есть аккаунт? 
        <router-link to="/login" class="text-green-400 hover:underline">Войти</router-link>
      </p>
    </div>
  </div>
</template>
